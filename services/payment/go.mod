...
//
// testkit is in `replace` but NOT in `require`: it is used only by the contract- and
// integration-tagged tests, which run in workspace mode where go.work provides it. Keeping it out
// of `require` keeps its testcontainers dependency tree out of THIS module's go.sum — the same
// shape booking uses.
replace (
