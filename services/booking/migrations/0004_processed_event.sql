...
--
-- Booking consumes PaymentSucceeded, PaymentFailed, and ReservationReleased, and the AsyncAPI
-- contract makes delivery at-least-once with consumers required to dedupe on the envelope id.
-- inbox.Process writes a row here in the SAME transaction as the handler's writes; see the inbox
-- package doc for why any arrangement of two separate transactions either loses work or
-- double-processes it — and for a payment, double-processing is a double charge's worth of damage.
--
...
CREATE TABLE processed_event (
  ...
  -- delivered to several services. Keying on event_id alone would let whichever consumer ran
  -- first suppress the event for all the others.
