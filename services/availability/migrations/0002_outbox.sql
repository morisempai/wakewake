...
--
-- This DDL is the canonical definition from shared/platform/outbox/schema.sql, pasted verbatim.
-- shared/ cannot own another service's migrations (hard rule #6), so the one authoritative copy
-- lives there as text and each service pastes it. Do NOT hand-edit the columns: outbox.Enqueue
-- and the relay write and read exactly these names.
