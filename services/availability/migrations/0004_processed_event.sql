...
--
-- Availability consumes BookingCancelled (the compensation path), and the AsyncAPI contract makes
-- delivery at-least-once with consumers required to dedupe on the envelope id. inbox.Process
-- writes a row here in the SAME transaction as the handler's writes; see the inbox package doc
-- for why any arrangement of two separate transactions either loses work or double-processes it.
--
